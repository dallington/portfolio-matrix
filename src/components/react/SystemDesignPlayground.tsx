...
export const SystemDesignPlayground = () => {
	...
	useEffect(() => {
		setWorkers((prev) =>
			prev.map((w, i) => ({ ...w, active: i < workerCount })),
		);
	}, [workerCount]);
	...
	useEffect(() => {
		...
		const interval = setInterval(() => {
			setJobs((prevJobs) => {
				...
				workers
					.filter((w) => w.active && !w.currentJobId)
					.forEach((worker) => {
				...
				if (nextJobs.filter((j) => j.status === "queued").length < 5) {
					const lastId =
						nextJobs.length > 0 ? nextJobs[nextJobs.length - 1].id : 0;
					nextJobs.push({
						id: lastId + 1,
						status: "queued",
						workerId: null,
	...
	return (
		<div
			className={`p-8 border-2 transition-all ${isDev ? "bg-black border-[var(--accent)]/20 mono" : "bg-gray-50 dark:bg-[#0D0D0D] border-gray-100 dark:border-[#1A1A1A] rounded-2xl"}`}
		>
						...
						<AnimatePresence>
							{jobs.slice(-50).map((job) => (
								...
								>
									...
									{job.status === "processing" && (
										<div
											className="absolute inset-0 bg-yellow-500/20 origin-bottom transition-all"
					...
					<section>
						...
						<div className="space-y-4">
							{[
								{
									label: "THROUGHPUT",
									value: `${throughput} jobs/s`,
									color: isRunning ? "text-green-500" : "text-gray-500",
								},
								{
									label: "COMPLETED",
								...
								<div
									key={m.label}
									className="flex justify-between items-end border-b border-gray-100 dark:border-[#1A1A1A] pb-2"
								>
					...
					<section className="space-y-6">
						...
						<div className="space-y-2">
							...
							<div className="flex justify-between gap-1 mt-2">
								{workers.map((w) => (
									<div
										key={w.id}
										className={`flex-1 h-6 border flex items-center justify-center rounded-sm transition-all ${w.active ? (w.currentJobId ? "bg-yellow-500/20 border-yellow-500" : "bg-[var(--accent)]/10 border-[var(--accent)]/40") : "bg-gray-100 dark:bg-gray-900 border-transparent opacity-20"}`}
									>
										<Server
											size={10}
											className={
						...
						<div className="grid grid-cols-1 gap-3">
							...
							>
								{isRunning ? (
									<>
										<Pause size={14} /> Shutdown_Systems
									</>
								) : (
									<>
										<Play size={14} /> Initialize_Cluster
									</>
								)}
							</button>
						</div>
					...
					{isDev && (
						<div className="pt-6 border-t border-gray-800">
							<div className="text-[9px] text-gray-400 italic space-y-2 leading-relaxed">
								<p>
									// Log: Worker pool uses "FOR UPDATE SKIP LOCKED" to prevent
									race conditions during job fetch.
								</p>
								<p>
									// Log: Redis Pub/Sub synchronizes state changes across
