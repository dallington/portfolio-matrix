...
export const TechnicalDNA = () => {
  ...
  return (
    <section id="about-preview" className={`py-24 px-6 border-y border-gray-200 dark:border-[#1A1A1A] transition-colors duration-500 ${isDev ? 'bg-gray-50 dark:bg-[#0D0D0D]' : 'bg-white dark:bg-[#0A0A0A]'}`}>
      <div className="max-w-7xl mx-auto">
        <div className="mb-20">
          {isDev && <div className="text-[var(--accent)] mono mb-4">// SKILL_SPECTRUM</div>}
          <h2 className={`text-5xl font-black text-gray-900 dark:text-white uppercase tracking-tighter ${!isDev && 'normal-case tracking-normal'}`}>
          ...
          {Object.entries(groupedSkills).map(([category, skills], idx) => (
            <div key={category} className="space-y-6">
              <div className="flex items-center gap-4">
                <span className={`text-xs font-black mono ${isDev ? 'text-[var(--accent)]' : 'text-blue-600'}`}>
                  {isDev ? `0x0${idx + 1}` : `[${idx + 1}]`}
                </span>
                <h3 className={`text-md font-black text-gray-900 dark:text-white uppercase tracking-wider ${isDev ? 'mono' : 'font-sans'}`}>
                ...
                {skills.map((skill) => (
                  ...
                  >
                    <div className="flex items-center gap-4 relative z-10">
                      <div className={`p-2 transition-colors duration-300 ${isDev ? 'bg-gray-50 dark:bg-[#1A1A1A] text-gray-400 group-hover:text-[var(--accent)]' : 'bg-gray-50 dark:bg-[#1A1A1A] text-blue-600 dark:text-blue-400'}`}>
                        {skill.icon}
                      </div>
                      <div>
                        <h4 className={`font-bold text-gray-900 dark:text-white ${isDev ? 'mono text-xs' : 'font-sans'}`}>{skill.name}</h4>
                        {skill.subcategory && (
