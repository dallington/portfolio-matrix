...
import { ChevronLeft, Calendar, Clock, List } from 'lucide-react';
...
export const BlogLayout = ({ post, onBack }: BlogLayoutProps) => {
  const { isDev } = useMode();
    ...
    <article className="pt-32 pb-24 px-6 min-h-screen bg-white dark:bg-[#0A0A0A] transition-colors duration-500">
      ...
      <div className="max-w-7xl mx-auto grid lg:grid-cols-[1fr_300px] gap-16">
        <div className="min-w-0">
          <button onClick={handleBack} className={`flex items-center gap-2 mb-12 font-bold transition-all ${isDev ? 'mono text-xs text-[var(--accent)] hover:-translate-x-2' : 'font-sans text-blue-600 hover:text-blue-700'}`}>
          ...
          <header className="mb-16">
            <div className="flex items-center gap-4 mb-6">
              {post.data.tags.map(t => (
                <span key={t} className="text-[10px] mono text-gray-400 uppercase tracking-[0.2em]">#{t}</span>
              ))}
            </div>
            <h1 className={`text-4xl md:text-7xl font-black text-gray-900 dark:text-white leading-[1.1] mb-8 ${isDev ? 'mono uppercase' : 'font-sans'}`}>
              {post.data.title}
            </h1>
            <div className="flex items-center gap-8 py-6 border-y border-gray-100 dark:border-[#1A1A1A]">
              <div className="flex items-center gap-2">
                <Calendar size={14} className="text-gray-400" />
                <span className={`text-xs font-bold uppercase mono ${isDev ? 'text-[var(--accent)]' : 'text-blue-600'}`}>
                  {new Date(post.data.date).toLocaleDateString()}
                </span>
              </div>
              <div className="flex items-center gap-2">
                <Clock size={14} className="text-gray-400" />
                <span className="text-xs font-bold uppercase mono text-gray-500">{post.data.readTime}</span>
              </div>
            </div>
          </header>

          <div className={`prose prose-lg dark:prose-invert max-w-none transition-colors duration-500 mb-24 text-gray-600 dark:text-gray-400  ${isDev ? 'font-mono' : 'font-sans'}`}>
            {content.split('\n').map((line, i) => {
              const text = line.replace(/^#+\s*/, '').replace(/[\*\_\[\]]/g, '');
              const id = text.toLowerCase().replace(/[^\w\s-]/g, '').replace(/\s+/g, '-');
              
              if (line.startsWith('# ')) return <h1 id={id} key={i} className="text-4xl font-black mb-8 mt-12 scroll-mt-32">{text}</h1>;
              if (line.startsWith('## ')) return <h2 id={id} key={i} className="text-3xl font-black mb-6 mt-10 border-b border-gray-100 dark:border-[#222] pb-2 scroll-mt-32">{text}</h2>;
              if (line.startsWith('### ')) return <h3 id={id} key={i} className="text-xl font-bold mb-4 mt-8 scroll-mt-32">{text}</h3>;
              if (line.startsWith('```')) {
                ...
                return <div key={i} className="mono text-[10px] uppercase text-gray-500 mb-2 mt-4 tracking-widest">Code Block / Source</div>;
              }
              if (line.trim() === '') return <div key={i} className="h-4" />;
              
              // Basic paragraph rendering
              return <p key={i} className="text-gray-600 dark:text-gray-400 leading-relaxed mb-6">{line}</p>;
            })}
          </div>
        ...
        <aside className="hidden lg:block space-y-8">
          <div className={`p-6 border transition-colors rounded-xl ${isDev ? 'bg-black text-[var(--accent)] border-[var(--accent)]/30 mono' : 'bg-gray-50 dark:bg-[#111] dark:border-[#222]'}`}>
            <h4 className="text-xs font-black uppercase mb-4 tracking-widest text-gray-500">Article Meta</h4>
            <div className="space-y-4 text-xs">
              ...
              <div className="flex justify-between">
                ...
                <span className="font-bold dark:text-white">{post.data.tags.length}</span>
              </div>
            </div>
          </div>
          {tableOfContents.length > 0 && (
            <div className={`sticky top-32 p-6 border transition-all rounded-xl ${isDev ? 'bg-black border-[var(--accent)]/30 mono' : 'bg-gray-50 dark:bg-[#111] dark:border-[#222]'}`}>
              ...
              <nav className="space-y-4">
                {tableOfContents.map((item, idx) => (
                  <button key={idx} onClick={() => scrollToId(item.id)} className={`block text-left text-xs transition-all border-l-2 pl-3 ${activeId === item.id ? (isDev ? 'text-[var(--accent)] border-[var(--accent)] font-black' : 'text-blue-600 border-blue-600 font-black') : 'text-gray-400 border-transparent hover:text-gray-900 dark:hover:text-white'} ${item.level === 1 ? 'uppercase' : 'ml-2'}`}>
                    {item.text}
                  </button>
                ))}
              </nav>
