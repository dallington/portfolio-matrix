...
import { Sun, Moon, Monitor, User, Search, Menu, X } from 'lucide-react';
    ...
    <>
      <header className={`fixed top-0 left-0 w-full z-50 border-b transition-colors duration-500 ${isDev ? 'border-gray-200 dark:border-[#1A1A1A] bg-white/80 dark:bg-[#0A0A0A]/80' : 'border-gray-100 dark:border-[#1A1A1A] bg-white/95 dark:bg-[#0A0A0A]/95'} backdrop-blur-md px-6 py-4`}>
        <div className="max-w-7xl mx-auto flex justify-between items-center">
          <a href="/" className="flex items-center gap-2 cursor-pointer z-50 relative">
          ...
          <div className="flex items-center gap-4 md:gap-8 z-50 relative">
            ...
            <div className="flex items-center gap-2 border-l border-gray-200 dark:border-[#222] pl-4 md:pl-8">
              ...
              >
                ...
                <span className="inline">{isDev ? 'DEV_MODE_ON' : 'RECRUITER_MODE'}</span>
              </button>
        ...
        >
          <nav className={`flex flex-col items-center gap-8 text-2xl font-bold ${isDev ? 'mono' : 'font-sans'}`}>
            <a href="/#work" onClick={() => setIsMenuOpen(false)} className={`hover:text-[var(--accent)] transition-colors text-gray-900 dark:text-white ${!isDev && 'hover:text-blue-600'}`}>WORK</a>
            <a href="/labs" onClick={() => setIsMenuOpen(false)} className={`hover:text-[var(--accent)] transition-colors text-gray-900 dark:text-white ${!isDev && 'hover:text-blue-600'}`}>LAB</a>
            <a href="/resume" onClick={() => setIsMenuOpen(false)} className={`hover:text-[var(--accent)] transition-colors text-gray-900 dark:text-white ${!isDev && 'hover:text-blue-600'}`}>RESUME</a>
            <a href="/about" onClick={() => setIsMenuOpen(false)} className={`hover:text-[var(--accent)] transition-colors text-gray-900 dark:text-white ${!isDev && 'hover:text-blue-600'}`}>ABOUT</a>
            <a href="/blog" onClick={() => setIsMenuOpen(false)} className={`hover:text-[var(--accent)] transition-colors text-gray-900 dark:text-white ${!isDev && 'hover:text-blue-600'}`}>BLOG</a>
          </nav>
        </div>
      </header>
    </>
